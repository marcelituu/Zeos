...
int access_ok(int type, const void * addr, unsigned long size)
{
  ...
  //if addr is from shared page is ok

   //Todo lo relacionado con las shared pages
   int paginas= (addr_fin - addr_ini) +1;
  page_table_entry *pt=get_PT(current());
  int its_ok=0;
  for (int i=0; i<paginas; i++){
    int frame = get_frame(pt, addr_ini+i);
    for (int j=0; j<NUM_SHRD_PAGES; j++){
      if (shared_pages[j].frame_id == frame){
        ++its_ok;
      }
    }
  }
